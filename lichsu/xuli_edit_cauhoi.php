...
if($_SERVER['REQUEST_METHOD'] != 'POST'){
    header('location: '. $ADMIN_URL .'lichsu/qlch6.php');
    die;
}
$page = $_POST['page'];
...
if ($path != "") {      
    ...
    $filename = "img/lichsu/" . uniqid() . "." . $ext; //tạo ra tên file duy nhất cho ảnh
    ...
    if($nd !="" || $a != "" || $b !="" || $c !="" || $d !="" || $i !=""){
        header('location:'.$ADMIN_URL.'lichsu/themcauhoi6.php?notsuccess=true'.'&&'.$nd.$a.$b.$c.$d.$i);
        die;
    }
if ($filename != "") {
    # code...
    ...
    $sql="update cauhoi_ls set ngaythem ='$ngaythem', cauhoi='$noidung', dapanA='$dapanA', dapanB='$dapanB', dapanC='$dapanC',dapanD='$dapanD', dapandung=$dapandung, dokho=$dokho where id_cauhoi = $id_cauhoi";
    $run=getSimpleQuery($sql);



if ($linkanhcu != $file && $filename!= "") {
    $sql="update cauhoi_ls set hinhanh='$filename' where id_cauhoi = $id_cauhoi";
    $run=getSimpleQuery($sql);
   }   


    header('location: '. $ADMIN_URL . 'lichsu/qlch6.php?page='.$page.'&&editsuccess=true');
    die;
