...
$sql = "SELECT * FROM cauhoi_hoa where id_cauhoi = $id";
                      ...
                      <div class="card-body">
                        ...
                        <form class="forms-sample" method="post" action="xuli_edit_cauhoi.php" enctype="multipart/form-data">
                          ...
                          <div class="form-group">
                            ...
                            if ($kq['amin_protein']>0) {
                              $loaicau_cu=6;
                            }
                            if ($kq['polime']>0) {
                              $loaicau_cu=7;
                            }
                            if ($kq['daicuongkimloai']>0) {
                              $loaicau_cu=8;
                            }
                            if ($kq['kimloai']>0) {
                              $loaicau_cu=9;
                            }
                            if ($kq['nhanbiethoahoc']>0) {
                              $loaicau_cu=10;
                            }
                            ?>
                            <input type="hidden" name="loaicau_cu" value="<?=$loaicau_cu?>">
                            <select class="form-control" name="loaicau" id="exampleSelectGender">
                              <option value="1" <?php if($kq['sudienli'] > 0){ echo 'selected'; } ?>> Sự Điện Li</option>
                              <option value="2" <?php if($kq['phanbonhhoahoc'] > 0){ echo 'selected'; } ?>> Phân Bón Hóa Học</option>
                              <option value="3" <?php if($kq['hidrocacbon'] > 0){ echo 'selected'; } ?>> Hidrocacbon</option>
                              <option value="4" <?php if($kq['este_lipit'] > 0){ echo 'selected'; }  ?>> Este - Lipit</option>
                              <option value="5" <?php if($kq['cacbohidrat'] > 0){ echo 'selected'; }  ?>> Cacbonhiđrat</option>
                              <option value="6" <?php if($kq['amin_protein'] > 0){ echo 'selected'; }  ?>> Amin - Protein</option>
                              <option value="7" <?php if($kq['polime'] > 0){ echo 'selected'; }  ?>> Polime</option>
                              <option value="8" <?php if($kq['daicuongkimloai'] > 0){ echo 'selected'; }  ?>> Đại Cương Kim Loại</option>
                              <option value="9" <?php if($kq['kimloai'] > 0){ echo 'selected'; }  ?>> Kim Loại</option>
                              <option value="10" <?php if($kq['nhanbiethoahoc'] > 0){ echo 'selected'; }  ?>> Nhận Biết Hóa Học</option>
                            </select>
                          </div>
                        ...
                        <div class="col-sm-4">
                          ...
                          <div class="form-group">
                            ...
                            <div>
                              <?php if ($kq['hinhanh']=="") {
                                echo "<img width='200px' height='150px' src='../img/hoahoc/default.jpg'>";
                              } else {
                          ...
                          <button type="button" class="btn btn-light" onclick="location.href='<?=$ADMIN_URL?>hoahoc/qlch3.php?page=<?=$page?>'">Cancel</button>
                        </form>
                      </div>
                    </div>
                  </div>
