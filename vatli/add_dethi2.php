...
if($_SERVER['REQUEST_METHOD'] != 'POST'){
    header('location: '. $ADMIN_URL .'vatli/qldethi2.php');
    die;
}
$scc1 = $_POST['loai1'];
$scc2 = $_POST['loai2'];
...
if($de !=""){
        header('location:'.$ADMIN_URL.'vatli/taodethi2.php?notsuccess=true'.'&&'.$de);
        die;
    }

$sql="SELECT * FROM cauhoi_ly WHERE daodong > 0 ORDER by rand() limit $scc1";
$sc1=getSimpleQuery($sql, true);
$sql="SELECT * FROM `cauhoi_ly` WHERE songco > 0 ORDER by rand() limit $scc2";
$sc2=getSimpleQuery($sql, true);
$sql="SELECT * FROM `cauhoi_ly` WHERE dien > 0 ORDER by rand() limit $scc3";
$sc3=getSimpleQuery($sql, true);
$sql="SELECT * FROM `cauhoi_ly` WHERE machgiaodong > 0 ORDER by rand() limit $scc4";
$sc4=getSimpleQuery($sql, true);
$sql="SELECT * FROM `cauhoi_ly` WHERE songanhsang > 0 ORDER by rand() limit $scc5";
$sc5=getSimpleQuery($sql, true);
$sql="SELECT * FROM `cauhoi_ly` WHERE luongtu > 0 ORDER by rand() limit $scc6";
$sc6=getSimpleQuery($sql, true);
$sql="SELECT * FROM `cauhoi_ly` WHERE hatnhan > 0 ORDER by rand() limit $scc7";
$sc7=getSimpleQuery($sql, true);
$sql="SELECT * FROM `cauhoi_ly` WHERE vatly11 > 0 ORDER by rand() limit $scc8";
