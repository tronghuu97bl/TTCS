...
if($_SERVER['REQUEST_METHOD'] != 'POST'){
    header('location: '. $ADMIN_URL .'diali/qlch7.php');
    die;
}
$page = $_POST['page'];
...
if ($path != "") {      
    ...
    $filename = "img/diali/" . uniqid() . "." . $ext; //tạo ra tên file duy nhất cho ảnh
    ...
    if($nd !="" || $a != "" || $b !="" || $c !="" || $d !="" || $i !=""){
        header('location:'.$ADMIN_URL.'diali/themcauhoi7.php?notsuccess=true'.'&&'.$nd.$a.$b.$c.$d.$i);
        die;
    }
if ($filename != "") {
    # code...
    ...
    $sql="update cauhoi_dia set ngaythem ='$ngaythem', cauhoi='$noidung', dapanA='$dapanA', dapanB='$dapanB', dapanC='$dapanC',dapanD='$dapanD', dapandung=$dapandung, dokho=$dokho where id_cauhoi = $id_cauhoi";
    $run=getSimpleQuery($sql);



if ($linkanhcu != $file && $filename!= "") {
    $sql="update cauhoi_dia set hinhanh='$filename' where id_cauhoi = $id_cauhoi";
    $run=getSimpleQuery($sql);
   }   


    header('location: '. $ADMIN_URL . 'diali/qlch7.php?page='.$page.'&&editsuccess=true');
    die;
